...
START:
    ...
    MOVLW 0X00		            ; Put literal value 0x00 into WREG
    MOVWF TRISD		            ; Move the value of W to TRISD. 00 makes it so that all of PORTC is seen as outputs.
   
MAIN_LOOP:		                ; start of While LOOP
    MOVLW 0x00		            ; Put literal value 0x00 into WREG
    MOVWF 0x20		            ; Move the value of WREG into the address 0x20. This will be the color of the LED.
...
FOR_LOOP:
    MOVF 0x20,W		            ; read content of 0x20, which is the color, into W.
    MOVWF PORTC		            ; the value of W is then moved into PORTC, where PORTC will output the colors.
    
    SUBLW 0x07                  ; subtract 7 from W, which was loaded in line 26. 
    MOVWF PORTD		            ; the subtracted value should count down and the color will output to PORTD.
    
    CALL DELAY_ONE_SEC	        ; wait one second
    INCF 0x20,F		            ; increment location 0x20 by 1, changing the colors of PORTC and PORTD.
