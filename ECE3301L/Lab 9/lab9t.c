...
void Day_Mode()
{
    ...
    if (NS_PED_SW == 1) {PED_Control(NS,7);}
    
    Wait_N_Seconds(9);                              // day step 2
    
    Set_NS(YELLOW);                                 // day step 3
    Wait_N_Seconds(3);
    
    Set_NS(RED);                                    // day step 4
    
    if (EW_LT_SW == 1)                               // day step 5
    {
        Set_EW_LT(GREEN);                            // day step 6
        Wait_N_Seconds(7);
        
        Set_EW_LT(YELLOW);                           // day step 7
        Wait_N_Seconds(3);
        
        Set_EW_LT(RED);                              // day step 8
    }
    
    Set_EW(GREEN);                                  // day step 9
    if (EW_PED_SW == 1) {PED_Control(EW,9);}
    
    Set_EW(GREEN);                                  // day step 10
    Wait_N_Seconds(8);
    
    Set_EW(YELLOW);                                 // day step 11
    Wait_N_Seconds(3);
    
    Set_EW(RED);                                    // day step 12
    
    if (NS_LT_SW == 1)                               // day step 13
    {
        Set_NS_LT(GREEN);                            // day step 14
        Wait_N_Seconds(7);
        
        Set_NS_LT(YELLOW);                           // day step 15
        Wait_N_Seconds(3);
        
        Set_NS_LT(RED);                              // day step 16
    }
}
void Night_Mode()
{ 
    ...
    Wait_N_Seconds(9);                              // night step 2
    
    Set_NS(YELLOW);                                 // night step 3
    Wait_N_Seconds(3);
    
    Set_NS(RED);                                    // night step 4
    
    if (EW_LT_SW == 1)                               // night step 5
    {
        Set_EW_LT(GREEN);                            // night step 6
        Wait_N_Seconds(7);
        
        Set_EW_LT(YELLOW);                           // night step 7
        Wait_N_Seconds(3);
        
        Set_EW_LT(RED);                              // night step 8
    }
    
    Set_EW(GREEN);                                  // night step 9
    Wait_N_Seconds(9);
    
    Set_EW(YELLOW);                                 // night step 10
    Wait_N_Seconds(3);
    
    Set_EW(RED);                                    // night step 11
    
    if (NS_LT_SW == 1)                               // night step 12
    {
        Set_NS_LT(GREEN);                            // night step 13
        Wait_N_Seconds(7);
        
        Set_NS_LT(YELLOW);                           // night step 14
        Wait_N_Seconds(3);
        
        Set_NS_LT(RED);                              // night step 15
    }
...
void update_LCD_color(char direction, char color)
{
    ...
    if (color == Color_Green)     //if the color is yellow only fill the red circle with red
    {
            fillCircle(XRED, Circle_Y, Circle_Size, ST7735_BLACK);
            fillCircle(XYEL, Circle_Y, Circle_Size, ST7735_BLACK);
